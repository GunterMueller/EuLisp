...
defmodule
init
(arith bci lists classes sockets streams ccc symbols strings calls others tables vectors (except (error cerror) errors) (only (set-compute-and-apply-fn generic-function-p methodp call-method-by-list) generics) class-names)
()
(expose arith bci lists classes sockets streams ccc symbols strings calls others tables vectors (except (error cerror) errors) (only (set-compute-and-apply-fn generic-function-p methodp call-method-by-list) generics) class-names)
(export <object>)
...
(defun %compute-reader (n) (if (i-greaterp 10 n) (vector-ref slot-readers n) (lambda (***method-status-handle*** ***method-args-handle*** x) (primitive-slot-ref x n))))
(defun %compute-writer (n) (if (i-greaterp 10 n) (vector-ref slot-writers n) (lambda (***method-status-handle*** ***method-args-handle*** x v) (primitive-set-slot-ref x n v))))
(defun make-initial-table (key entry) (mk-tab-aux key entry))
(defun mk-tab-aux (key entry) ((lambda (add-part) (setq add-part (lambda (lst tab) (if (null lst) tab (add-part (cdr lst) (cons (cons (car lst) tab) nil))))) (add-part (reverse key) entry)) ()))
(defun add-table-entry (table key value) (if (null table) (error "Can't happen" <Internal-Error>) ((lambda (xx) (if (null xx) (progn (nconc table (make-initial-table key value)) table) (if (null (cdr key)) (primitive-set-slot-ref-1 xx value) (add-table-entry (cdr xx) (cdr key) value)))) (assq (car key) table))))
(defun symbol-unbraced-name (sym) ((lambda (x) (if (eq (string-ref x 0) #\<) (substring x 1 (i-sub1 (i-sub1 (string-length x)))) x)) (symbol-name sym)))
(export symbol-unbraced-name)
(defun scan-args (arg init-lst panic) ((lambda (scan-aux) (setq scan-aux (lambda (arg lst) (if (null lst) (panic arg init-lst) (if (eq (car lst) arg) (car (cdr lst)) (scan-aux arg (cdr (cdr lst))))))) (scan-aux arg init-lst)) ()))
...
(defun simple-find-accessor (class slot-name) ((lambda (reader writer) (primitive-set-slot-ref-9 reader writer) reader) (simple-find-slot-reader class slot-name) (simple-find-slot-writer class slot-name)))
(defun initialize-slots (lst) (if (null lst) nil ((lambda (class slots) (primitive-set-slot-ref-5 class (append (if (null (primitive-slot-ref-2 class)) nil (primitive-slot-ref-5 (car (primitive-slot-ref-2 class)))) (make-slot-list class slots))) (primitive-set-slot-ref-4 class (mapcar1 (lambda (sd) nil) (primitive-slot-ref-5 class))) (primitive-set-slot-ref-6 class (primitive-slot-ref-5 class)) (initialize-slots (cdr lst))) (car (car lst)) (cdr (car lst)))))
(defun make-slot-list (class slotds) (if (null slotds) nil ((lambda (slotd slot) (fill-slot-description slot class (cons (car slotd) (cons (primitive-slot-ref-1 class) (cdr slotd)))) (cons slot (make-slot-list class (cdr slotds)))) (car slotds) (allocate-object (car (car slotds))))))
...
(defun init-generic (gf) ((lambda (lookup) (primitive-set-slot-ref-7 gf lookup) (primitive-set-slot-ref-1 gf (std-generic-discriminator gf lookup)) (primitive-set-slot-ref-6 gf (cons <method> nil))) (simple-compute-method-lookup-function gf nil)))
(defun add-method-to-caches (gf sig meths) (primitive-set-slot-ref-3 gf (cons sig meths)) ((lambda (table) (if (null table) (primitive-set-slot-ref-4 gf (make-initial-table sig (cons sig meths))) (add-table-entry table sig (cons sig meths)))) (primitive-slot-ref-4 gf)))
...
(defun find-applic-methods-aux (table cpl-lst) (if (null cpl-lst) nil (if (null (car cpl-lst)) nil ((lambda (xx) (if (null xx) (find-applic-methods-aux table (cons (cdr (car cpl-lst)) (cdr cpl-lst))) (if (null (cdr cpl-lst)) (if (methodp (car (cdr xx))) (cons (car (cdr xx)) (find-applic-methods-aux table (cons (cdr (car cpl-lst)) (cdr cpl-lst)))) (progn (cerror "yowzer" <Internal-Error> (quote error-value) xx))) (append (find-applic-methods-aux (cdr xx) (cdr cpl-lst)) (find-applic-methods-aux table (cons (cdr (car cpl-lst)) (cdr cpl-lst))))))) (assq (car (car cpl-lst)) table)))))
(deflocal debug nil)
(defun find-and-call-generic (gf args) ((lambda (meths sig) (if (null meths) (error "No applicable methods" no-applicable-method (quote error-value) gf (quote sig) sig (quote args) args) ((lambda (trimmed-sig) (add-method-to-caches gf trimmed-sig meths) (call-method-by-list meths args)) (trim-signature gf sig)))) ((stable-generic-lookup-function gf) args) (mapcar1 class-of args)))
(defun restrict-method (gf sig) ((lambda (domain) (if (null domain) sig ((lambda (restrict-lsts) (setq restrict-lsts (lambda (sig dom) (if (null sig) nil (if (subclassp (car sig) (car dom)) (cons (car sig) (restrict-lsts (cdr sig) (cdr dom))) (if (subclassp (car dom) (car sig)) (cons (car dom) (restrict-lsts (cdr sig) (cdr dom))) (progn (error "Add-method: outside domain" <Internal-Error>) 2)))))) (restrict-lsts sig domain)) ()))) (generic-method-domain gf)))
...
(add-method generic-write (make <method> (quote signature) (list <object> <object>) (quote function) (lambda (***method-status-handle*** ***method-args-handle*** x y) (generic-prin x y))))
(add-method flush (make <method> (quote signature) (list <object>) (quote function) (lambda (***method-status-handle*** ***method-args-handle*** y) nil)))
(deflocal no-applicable-method ())
(defun set-no-applicable-method (x) (setq no-applicable-method x))
(export set-no-applicable-method)
(add-method allocate (make <method> (quote signature) (list <primitive-class> <object>) (quote function) (lambda (***method-status-handle*** ***method-args-handle*** c l) (error "Cannot allocate primitive class" <Internal-Error> (quote error-value) c))))
...
(add-method copy (make <method> (quote signature) (list <cons>) (quote function) (lambda (***method-status-handle*** ***method-args-handle*** x) (cons (car x) (cdr x)))))
(add-method copy (make <method> (quote signature) (list (class-of nil)) (quote function) (lambda (***method-status-handle*** ***method-args-handle*** x) nil)))
(add-method copy (make <method> (quote signature) (list <structure>) (quote function) (lambda (***method-status-handle*** ***method-args-handle*** x) ((lambda (copy-slots) (setq copy-slots (lambda (old new slot-list) (if (null slot-list) nil (progn ((slot-description-slot-writer (car slot-list)) new ((slot-description-slot-reader (car slot-list)) new)) (copy-slots old new (cdr slot-list)))))) (copy-slots x (allocate (class-of x) nil) (class-slot-descriptions x))) ()))))
...
(add-method generic-hash (make <method> (quote signature) (list <i-function>) (quote function) (lambda (***method-status-handle*** ***method-args-handle*** x) 99)))
(add-method generic-hash (make <method> (quote signature) (list <object>) (quote function) (lambda (***method-status-handle*** ***method-args-handle*** x) 0)))
(set-standard-tab-functions generic-hash eq)
(export generic-hash)
(defconstant i-function-setters (make-table ()))
(defconstant i-setter (lambda (***method-status-handle*** ***method-args-handle*** x) ((lambda (xx) (if (functionp xx) xx (error "Setter: no setter for function" <Internal-Error> (quote error-value) x))) (sys-table-ref i-function-setters x))))
(defconstant i-setter-setter (lambda (***method-status-handle*** ***method-args-handle*** x y) (if (if (functionp x) (if (functionp y) t nil) nil) ((setter sys-table-ref) i-function-setters x y) (error "Bad setter" <Internal-Error> (quote error-value) (cons x y)))))
...
(defconstant error (lambda (message type . junk) ((lambda (lst) (internal-signal (initialize (allocate type lst) lst) nil)) (cons (quote message) (cons message junk)))))
(defconstant cerror (lambda (message type . junk) ((lambda (lst) (simple-call/cc (lambda (cont) (internal-signal (initialize (allocate type lst) lst) cont)))) (cons (quote message) (cons message junk)))))
(export error cerror)
